...
class Caption_Generator():
    ...
    def __init__(self, config, data=None):
        ...
        if self.mode == 'train':
            ...
            if self.resume == 1:
                self.vocab = np.load("Data/vocab.npy").tolist()
                self.wtoidx = np.load("Data/wordmap.npy").tolist()
    ...
    def Words_to_IDs(self, wtoidx, caption_batch):   ## replace each word of a caption by its index 
    ...
    def build_decode_graph(self):
        ...
        with open("Results/DecoderOutputs.txt", 'w') as f:
            for name in IDs:
    ...
    def decode(self, path):
        ...
        if self.savedecoder:
            saver = tf.train.Saver()
            saver.save(self.sess, "Results/model.ckpt")
    ...
    def batch_decoder(self, filenames, features):
        ...
        with open("Results/Test_Generated_Captions.txt", 'w') as f:
            with tf.Session() as sess:
                init = tf.global_variables_initializer()
                sess.run(init)
                saver.restore(sess, ckpt_file)
                for i, feat in enumerate(features):
                    feat = np.reshape(feat, newshape=(1, 3072))
